...
$(document).ready(function(){
    ...
    //     autoclose: true
    // });
    $('.action_span_time').datepicker({
        format: "yyyy-mm-dd",
        startDate: "today",
        language: "zh-CN",
        autoclose: true
    });
    $('.end_span_time').datepicker({
        format: "yyyy-mm-dd",
        startDate: "today",
        language: "zh-CN",
        autoclose: true
    ...
    $(".filter-title-bar").on("click", ".filter-title", function(e){
        if(index==0){//选择的时间
            clearInterval(interval);//再次点击先停止
        }
        ...
        if(index==0){//选择的时间
            interval=setInterval(data_time_action, 100);
        }
    });
    ...
    $(".filter-item-bar>.filter-area").on("click", "li", function(e) {
        if (!$(this).hasClass("active")) {
            ...
            var url='/search?';
            url+='area_id='+$(this).attr('value');
            url+='&area_name='+$(this).html();
            url+='&start_date='+getUrlParam('start_date');
            url+='&end_date='+getUrlParam('end_date');
            url+='&sort_key='+getUrlParam('sort_key');
    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function(e) {
        if (!$(this).hasClass("active")) {
            ...
            var url='/search?';
            url+='area_id='+getUrlParam('area_id');
            url+='&area_name='+getUrlParam('area_name');
            url+='&start_date='+getUrlParam('start_date');
            url+='&end_date='+getUrlParam('end_date');
            url+='&sort_key='+$(this).attr('sort-key');
            window.location.href=url;
        }
    })
})

var interval=setInterval(data_time_action, 100);
function data_time_action(){
    var action_time=$('#start-date').val();
    var end_time=$('#end-date').val();
    if(action_time!='' && end_time!=''){
        //area_id=&area_name=&start_date=&end_date=
        var url='/search?';
        url+='area_id='+getUrlParam('area_id');
        url+='&area_name='+getUrlParam('area_name');
        url+='&start_date='+action_time;
        url+='&end_date='+end_time;
        url+='&sort_key='+getUrlParam('sort_key');
        window.location.href=url;
    }
}
